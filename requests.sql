...
WITH RECURSIVE order_chain AS (
    -- Базовый уровень: начальный статус каждого заказа
    SELECT 
        order_id,
        status,
        changed_at,
    ...
    -- Рекурсивная часть: следующий статус
    SELECT 
        h.order_id,
        h.status,
        h.changed_at,
        ...
        oc.step + 1,
        oc.start_time
    FROM st.order_status_history h
    INNER JOIN order_chain oc ON h.order_id = oc.order_id
    WHERE h.changed_at > oc.changed_at
      AND NOT EXISTS (
          SELECT 1 
          FROM st.order_status_history h2 
          WHERE h2.order_id = h.order_id 
            AND h2.changed_at > oc.changed_at 
